 ...
 * @LastEditTime: 2023-02-24 16:13:16
 * @LastEditors: szf
 * @Description: 运动学逆解算及PID计算函数
 * @FilePath: \ER\Usercode\user_src\usercalculate.c
 ...
 * @param 
 * @return 
 * @bug 转换方程还不确定，需要测试定位系统之后确定
 */
mavlink_control_set_t Direction_Translate(mavlink_control_set_t *control,mavlink_posture_t *posture)
{
    mavlink_control_set_t result;
    result.vx_set = control->vx_set * sin(posture->zangle) + control->vy_set * cos(posture->zangle);
    result.vy_set = control->vx_set * cos(posture->zangle) + control->vy_set * sin(posture->zangle);
    return result;
}